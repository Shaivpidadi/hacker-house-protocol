...
import { User, Wallet, Shield, Github, Linkedin, Settings, LogOut, Copy, ExternalLink } from "lucide-react"
...
export default function ProfilePage() {
  ...
  const walletAddress = "0x742d35Cc6634C0532925a3b8D404fddF4f5C456F"
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="glass-card border-b border-border sticky top-0 z-10">
        <div className="max-w-md mx-auto p-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold gradient-primary bg-clip-text text-transparent">Profile</h1>
      ...
      <div className="max-w-md mx-auto p-4 space-y-6">
        ...
        <div className="glass-card rounded-lg shadow-lg p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-foreground">{user?.name || "Anonymous Hacker"}</h2>
              <p className="text-muted-foreground">{user?.email || "hacker@cryptoreal.com"}</p>
            </div>
          </div>
          ...
          <div className="bg-primary/10 glass-card rounded-lg p-4 mb-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Talent Protocol Reputation</p>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold gradient-primary bg-clip-text text-transparent">{builderScore}</div>
          ...
          <div className="border border-border glass-card rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              {walletConnected ? (
                <span className="px-2 py-1 bg-chart-3/20 text-chart-3 text-xs rounded-full font-medium">Connected</span>
              ) : (
                <Button size="sm" onClick={connectWallet} className="gradient-primary text-primary-foreground glass">
                  Connect
                </Button>
              )}
            </div>
            {walletConnected && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <Button size="sm" variant="ghost" onClick={copyAddress} className="hover:bg-muted">
        ...
        <div className="glass-card rounded-lg shadow-lg p-6">
          ...
          <div className="space-y-3">
            {credentials.map((credential, index) => (
              ...
              >
                <div>
                  <div className="font-medium text-foreground">{credential.type}</div>
                  <div className="text-sm text-muted-foreground">Issued by {credential.issuer}</div>
                </div>
                <div className="flex items-center gap-2">
                  {credential.verified ? (
                    ...
                    <Button size="sm" variant="outline" className="text-xs glass-card border-border bg-transparent">
                      Verify
                    </Button>
                  )}
                </div>
        ...
        <div className="glass-card rounded-lg shadow-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 border border-border glass-card rounded-lg">
              ...
              <Button size="sm" variant="outline" className="glass-card border-border bg-transparent">
                Connect
              </Button>
            </div>
            <div className="flex items-center justify-between p-3 border border-border glass-card rounded-lg">
